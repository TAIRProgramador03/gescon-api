...
const login = async (req, res) => {
  ...
  try {
    const data = await withConnection(async (cn) => {
      ...
      if (result.length == 0) return { code: 404, message: "El usuario es incorrecto" };
      if (!result[0].CLV) return { code: 403, message: "El usuario no cuenta con contraseña" };
    ...
    if (data.code !== 200) return res.status(data.code).json({ success: false, message: data.message });
    ...
    res.status(200).json({ success: true, message: "Ingreso autorizado", permissions: data.permissions });
  } catch (error) {
    console.error("Error al iniciar sesión:", error);
    res.status(500).json({ success: false, message: "Ocurrio un error al intentar ingresar" });
  }
};
...
const verify = async (req, res) => {
  ...
  try {
    const usuario = await withConnection(async (cn) => {
      const sql = `SELECT U.V_TK FROM ${SCHEMA_BD}.T_US_GC U WHERE U.ID = ?`;
