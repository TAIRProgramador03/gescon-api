...
const detailVehByLeasing = async (req, res) => {
  ...
  try {
    const cleanedResult = await withConnection(async (cn) => {
      const sql = `
      SELECT L.MODELO, L.PLACA, L.CANTIDAD, L.TIPO_TERRENO, A.NROSER, V.ANO, V.COLOR, M.DESCRIPCION AS MARCA, O.DESCRIPCION AS OPERACION, COALESCE(A.CONDICION, '3') AS CONDICION, LC.NRO_LEASING, LC.FECHA_INI, LC.FECHA_FIN
